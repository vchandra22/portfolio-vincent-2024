...
@section('content')
    ...
    <section class="p-5 min-h-screen md:ml-64 bg-black">
        ...
        <form action="{{ route('backend.update_article', $detailArticle->id) }}" method="POST" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="title" id="title"
                        ...
                        placeholder="Input your title project" value="{{ old('title', $detailArticle->title) }}" required
                        autofocus />
                    @error('title')
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="8">{{ old('content', $detailArticle->content) }}</textarea>
                    @error('content')
                        <div class="mt-1 text-red-500 text-xs">
                            {{ $message }}
                        </div>
                ...
                <div>
                    ...
                    <div class="upload-img-box h-auto w-full md:max-w-sm">
                        @if ($detailArticle->gambar != null && $detailArticle->gambar != '')
                            <img class="h-56 w-full overflow-y-hidden border-abu-500 rounded-sm border-2"
                                src="{{ getImageFile($detailArticle->gambar) }}">
                        @else
                            <img src="{{ getImageFile($detailArticle->gambar) }}">
                        @endif
                        <input type="file" name="gambar" id="gambar" accept="image/*"
                            ...
                            value="{{ old('gambar', $detailArticle->gambar) }}" onchange="previewFile(this)">
                    </div>
                    @error('gambar')
                        <div class="mt-1 text-red-500 text-xs">
                            {{ $message }}
                ...
                <div class="pt-8">
                    ...
                    <input type="text" name="meta_title" id="meta_title"
                        class="bg-black border border-accent text-accent text-sm focus:ring-accent focus:border-accent block w-full p-2.5"
                        value="{{ old('meta_title', $detailArticle->meta_title) }}" />
                    @error('meta_title')
                        <div class="mt-1 text-red-500 text-xs">
                            {{ $message }}
                        </div>
                ...
                <div>
                    ...
                    <textarea name="meta_description" id="meta_description" rows="4"
                        class="bg-black border border-accent text-accent text-sm focus:ring-accent focus:border-accent block w-full p-2.5">{{ old('meta_description', $detailArticle->meta_description) }}</textarea>
                ...
                <div>
                    ...
                    <input type="text" name="meta_keyword" id="meta_keyword"
                        class="bg-black border border-accent text-accent text-sm focus:ring-accent focus:border-accent block w-full p-2.5"
                        value="{{ old('meta_keyword', $detailArticle->meta_keyword) }}" />
                    @error('meta_keyword')
                        <div class="mt-1 text-red-500 text-xs">
                            {{ $message }}
                        </div>
                ...
                <div>
                    ...
                    <div class="upload-img-box h-auto w-full md:max-w-sm">
                        @if ($detailArticle->og_image != null && $detailArticle->og_image != '')
                            <img class="h-56 w-full overflow-y-hidden border-abu-500 rounded-sm border-2"
                                src="{{ getImageFile($detailArticle->og_image) }}">
                        @else
                            <img src="{{ getImageFile($detailArticle->og_image) }}">
                        @endif
                        <input type="file" name="og_image" id="og_image" accept="image/*"
                            class="mt-2 w-full text-sm text-accent border border-secondary1 cursor-pointer bg-black hover:bg-secondary3 focus:ring-accent focus:border-accent"
                            value="{{ old('og_image', $detailArticle->og_image) }}" onchange="previewFile(this)">
                    </div>
                    @error('og_image')
                        <div class="mt-1 text-red-500 text-xs">
                            {{ $message }}
