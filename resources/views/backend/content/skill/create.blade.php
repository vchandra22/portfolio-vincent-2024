...
@section('content')
    ...
    <section class="p-5 min-h-screen md:ml-64 bg-black">
        ...
        <form action="{{ route('backend.store_skill') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="space-y-6">
                <div>
                    ...
                    <input type="text" name="gambar" id="gambar"
                        ...
                        placeholder="Describe your project in short" value="{{ old('gambar') }}" required autofocus />
                    @error('gambar')
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title"
                        class="bg-black border border-accent text-accent text-sm focus:ring-accent focus:border-accent block w-full p-2.5"
                        placeholder="Describe your project in short" value="{{ old('title') }}" required autofocus />
                    @error('title')
                        <div class="mt-1 text-red-500 text-xs">
                            {{ $message }}
                        </div>
                ...
                <div>
                    ...
                    <input type="text" name="description" id="description"
                        class="bg-black border border-accent text-accent text-sm focus:ring-accent focus:border-accent block w-full p-2.5"
                        placeholder="Describe your project in short" value="{{ old('description') }}" required autofocus />
                    @error('description')
                        <div class="mt-1 text-red-500 text-xs">
                            {{ $message }}
                        </div>
